'''
DEPRECATED - just use the FPKM data
...
if __name__ == '__main__':
    ...
    for i,dir in enumerate(os.listdir(path_in)):
        ...
        try:
            ...
            data2 = pd.read_csv(file_out,sep='\t')
            genes = data2.values[4:,0]

            ####################################################################
            ### Normalize data
            ####################################################################
            unstranded_counts = data2.values[4:,1]
            normalized_expession = (unstranded_counts - np.mean(unstranded_counts))/ np.std(unstranded_counts)
            D = {gene: [expr] for gene,expr in zip(genes, normalized_expession)}
