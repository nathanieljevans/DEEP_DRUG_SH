...
```{r}
CCLE.rnaSeq <- as.data.frame(fread('./../data/CCLE_expression.csv', sep=',')) %>% pivot_longer(-V1, names_to='gene') %>% mutate(cell_line = V1, expression = value) %>% select(-V1, -value) %>% separate(gene, into = c("HGNC_sym", "entrez_id"), sep = " \\(") %>% mutate(entrez_id=substr(entrez_id, 1, nchar(entrez_id)-1))
...
**NOTE:** beatAML expression data is in RPKM, but DepMap expression data is in TPM.   
...
beatAML.expr <- beatAML.expr %>% gather(-lab_id, key='ENSEMBL_GENE', value='FPKM_LOG2_EXPR')
...
beatAML.dat <- beatAML.drug %>% merge(beatAML.targetome.overlap, by.x='drug_name', by.y='CancerTargetome_Drug') %>% merge(targetome.evidence.full, by.x='InhibitorName_Panel', by.y='Drug') %>% filter(EvidenceLevel_Assigned %in% TIERS)  %>% select(drug_name, lab_id, response, response_type, Target_Name) %>% unique() %>% aggregate(Target_Name ~ drug_name + lab_id + response + response_type, data = ., paste, collapse = ";") %>% mutate(target = Target_Name, id=lab_id, id_type='lab_id') %>% select(-Target_Name, -lab_id)
...
write_delim(all.drug, './../data/processed/all_resp_data.csv', delim=',')
```
