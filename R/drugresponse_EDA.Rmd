...
## Expr gene variance ranking 

```{r}
CCLE.rnaSeq %>% head()

top.n.var.genes <- CCLE.rnaSeq %>% select(-entrez_id) %>% group_by(HGNC_sym) %>% summarize(expr_var=var(expression)) %>% arrange(desc(expr_var)) %>% head(20)

for (gene in top.n.var.genes$HGNC_sym) {
  print(gene)
}
```
